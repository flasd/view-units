...
function vUnit(options) {
  ...
  function createCSSRules() {
    ...
    var CSSRules = '';
    var value = 0; // Loop through all selectors passed on the CSSMap option

    Object.keys(map).forEach(function (selector) {
      var property = map[selector].property; // Adds rules from className1 to className100 to the stylesheet

      for (var range = 1; range <= 100; range += 1) {
  ...
  function appendStylesheetOnHead(stylesheet) {
    ...
    var head = document.head || document.getElementsByTagName('head')[0] || document.documentElement; // Grabs the previous stylesheet

    var legacyStylesheet = document.getElementById(vunit.options.stylesheetId); // Removes the previous stylesheet from the head, if any
  ...
  function viewportHasChanged() {
    ...
    var differentWidth = currentViewportSize.width !== vunit.viewportSize.width; // Updates the global letiable for future checking
  ...
  window.setInterval(function () {
    return requestAnimationFrame(viewportObserver);
